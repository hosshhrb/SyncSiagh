 ...
 * View Webhook Logs
 *
 * Shows recent webhook events received and their processing status
 * Usage: npx ts-node scripts/view-webhook-logs.ts [--count 20] [--failed] [--pending]
 */
  ...
  try {
    ...
    for (const log of logs) {
      ...
      if (log.triggerPayload) {
        ...
        const payload = log.triggerPayload as any;
